...
helixer_str_long <- pivot_longer(helixer_str, cols = B73_16DAP_embryo_MN01101:B73_V18_tassel_MN01062)
helixer_str_long$strand <- "stranded"
helixer_str_long$Annotation <- "helixer"

helixer_unstr <- fread("helixer.unstr.summary")
helixer_unstr_long <- pivot_longer(helixer_unstr, cols = B73_16DAP_embryo_MN01101:B73_V18_tassel_MN01062)
helixer_unstr_long$strand <- "unstranded"
helixer_unstr_long$Annotation <- "helixer"


nam.v5_str <- fread("v5.str2.summary")
nam.v5_str_long <- pivot_longer(nam.v5_str, cols = B73_16DAP_embryo_MN01101:B73_V18_tassel_MN01062)
nam.v5_str_long$strand <- "stranded"
nam.v5_str_long$Annotation <- "NAM.v5"

nam.v5_unstr <- fread("v5.unstr.summary")
nam.v5_unstr_long <- pivot_longer(nam.v5_unstr, cols = B73_16DAP_embryo_MN01101:B73_V18_tassel_MN01062)
...
myData <- rbind(helixer_str_long,
                ...
                nam.v5_str_long,
                nam.v5_unstr_long)
colnames(myData) <- c("fragment", "tissue", "count", "strand", "annotation")
...
myData.filtered.assigned <- myData.filtered %>%
  filter(fragment  ==  "Assigned" & strand == "stranded")
...
ggplot(myData.filtered, aes(x = tissue, y = percent, fill = fragment)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(title = "Percentage of Counts per Status by Assignment Type", x = "Tissue", y = "Percentage (%)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + facet_wrap(strand ~ annotation)
...
ggsave("assigned.png",  width = 12, height = 10, units = "in", dpi = 300)


ggplot(myData.filtered.unassigned, aes(x = tissue, y = percent, fill = annotation)) +
  geom_bar(stat = "identity", position = "dodge") +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        ...
        legend.position = "top",
        legend.key = element_blank())
ggsave("unassigned.png", width = 12, height = 10, units = "in", dpi = 300)
